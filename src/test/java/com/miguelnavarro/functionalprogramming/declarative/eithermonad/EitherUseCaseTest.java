...
class EitherUseCaseTest {
    ...
    void test(boolean successNif, boolean successPersonId, boolean successSave,
        ...
        eitherUseCase.storePurchase(PAN, 5.00)
            .peekLeft(error -> {
                assertThat(error.getPurchase().getStatus()).isEqualTo(finalStatus);
                Assertions.assertThat(error.getCode()).isNotNull();
                Assertions.assertThat(error.getMessage()).isEqualTo(finalStatus);
            })
            .peek(right -> {
                assertThat(right.getStatus()).isEqualTo(finalStatus);
                assertThat(right.getCard().getCardType()).isEqualTo(CardType.CREDIT);
                assertThat(right.getFee()).isEqualTo(5.00 * 0.04);
            });
